...
function dataAgent(url, data, mode){
   $.ajax({
      ...
      success: function(response){
    	  //alert(response); //object 타입
    	  console.log("위치: function dataAgent 리스폰스");
    	  console.log(response);
    	  
         switch(mode){
         case 1: displayRecommened(response); break;
         case 2: displayMyRecipe(response); break;         
         case 3: displayBookmark(response); break;
         case 4: displayMyPost(response); break;
         case 5: displayFollowing(response);
         }
      },
      error: function(error){
    	  console.log("X- data 에러 발생");
      }
   });
}
