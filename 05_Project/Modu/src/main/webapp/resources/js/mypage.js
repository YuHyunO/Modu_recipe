...
function displayRecommened(response){
   ...
   // console.log("##recipeList: "+recipeList); //문자열 넣으면 Object 타입으로만 보임 주의
	// ##recipeList: [object Object],[object Object]
   // console.log("##recipeList[0].mnickname: "+recipeList[0].mnickname); //닉네임
	// 정상 출력
   // console.log("##recipeList[0].mNickname: "+recipeList[0].mNickname); //닉네임
	// undefined
...
function displayMyRecipe(response){
   ...
   for(let item of recipeList){
      ...
      // html += '<a href="/recipe/delete?id='+item.id+'"
		// onclick="javascript:alert("정말 레시피를 삭제하시겠습니까?");"><button
		// type="button" class="gold-btn">삭제</button></a>';
      html += '</div>';
      html += '</div>';
      html += '</div>';      
   }
...
function displayMyPost(response){
   console.log("4 : displayMyPost 메소드 진입");
